...
void ObjectMgr::AddCreatureToGrid(uint32 guid, CreatureData const* data)
{
    ...
    for (uint32 i = 0; mask != 0; i++, mask >>= 1)
    {
        if (mask & 1)
        {
            CellCoord cellCoord = WoWSource::ComputeCellCoord(data->posX, data->posY);
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading Player Create Level Stats Data...");
    {
        ...
        for (int race = 0; race < MAX_RACES; ++race)
        {
            ...
            for (int class_ = 0; class_ < MAX_CLASSES; ++class_)
            {
                ...
                if (sWorld->getIntConfig(CONFIG_EXPANSION) < EXP_BC && (race == RACE_BLOODELF || race == RACE_DRAENEI))
                    continue;

                // skip expansion classes if not playing with expansion
                if (sWorld->getIntConfig(CONFIG_EXPANSION) < EXP_WOTLK && class_ == CLASS_DEATH_KNIGHT)
                    continue;

                // skip expansion classes / races if not playing with expansion
                if (sWorld->getIntConfig(CONFIG_EXPANSION) < EXP_CATACLYSM && (race == RACE_GOBLIN || race == RACE_WORGEN))

                // skip expansion classes / races if not playing with expansion
                if (sWorld->getIntConfig(CONFIG_EXPANSION) < EXP_PANDARIA && (class_ == CLASS_MONK || race == RACE_PANDAREN_NEUTRAL || race == RACE_PANDAREN_ALLI || race == RACE_PANDAREN_HORDE))
    ...
    do
    {
        ...
        switch (tmp.command)
        {
            ...
            case SCRIPT_COMMAND_TELEPORT_TO:
            {
                ...
                if (!WoWSource::IsValidMapCoord(tmp.TeleportTo.DestX, tmp.TeleportTo.DestY, tmp.TeleportTo.DestZ, tmp.TeleportTo.Orientation))
                {
            ...
            {
                if (!WoWSource::IsValidMapCoord(tmp.TempSummonCreature.PosX, tmp.TempSummonCreature.PosY, tmp.TempSummonCreature.PosZ, tmp.TempSummonCreature.Orientation))
                {
...
void ObjectMgr::LoadPointsOfInterest()
{
    ...
    do
    {
        ...
        if (!WoWSource::IsValidMapCoord(POI.x, POI.y))
        {
...
{
    //TODO: remove mail level rewards, we already have achievement reward
    return;
